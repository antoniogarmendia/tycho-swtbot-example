deploy_updates() {
    ...
    rm -rf out || true
    git clone -b gh-pages --depth 2 --single-branch https://github.com/antoniogarmendia/tycho-swtbot-example.git out
    cd out

    # Indicate clearly that this commit comes from Travis
    git config user.name "Travis CI"
    git config user.email "antonio.agj@gmail.com"

    # If the tip comes from Travis, amend it. Otherwise, add a new commit.
    rm -rf swtHelloworld-updates
    cp -r ../com.helloworld.tycho/releng/com.helloworld.tycho.update/target/repository swtHelloworld-updates
    git add --all .
